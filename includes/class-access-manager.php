...
class JPSM_Access_Manager
{
    ...
    public static function get_tier_name($tier)
    {
        if (class_exists('JPSM_Domain_Model')) {
            return JPSM_Domain_Model::get_tier_name($tier);
        }
    ...
    {
        if (class_exists('JPSM_Domain_Model')) {
            return JPSM_Domain_Model::get_tier_value($name);
        }
    ...
    public static function get_user_tier($email)
    {
        ...
        if (class_exists('JPSM_Data_Layer')) {
            ...
            $tiers = JPSM_Data_Layer::get_all_user_tiers();
            if (isset($tiers[$email_lower])) {
                $tier = intval($tiers[$email_lower]);
                error_log("[JPSM Access] Returning EXPLICIT tier: $tier for $email");
                return $tier;
            }
        } else {
        ...
        if ($calculated > self::TIER_DEMO) {
            ...
            error_log("[JPSM Access] Returning CALCULATED tier: $calculated for $email");
            return $calculated;
        }

        error_log("[JPSM Access] Returning DEFAULT tier: DEMO for $email");
        return self::TIER_DEMO;
    }
    ...
    public static function set_user_tier($email, $tier)
    {
        ...
        if (class_exists('JPSM_Data_Layer')) {
            return JPSM_Data_Layer::set_user_tier($email, intval($tier));
        }
    ...
    public static function register_lead($email)
    {
        ...
        if (class_exists('JPSM_Data_Layer')) {
            JPSM_Data_Layer::register_lead($email, 'mediavault_demo', current_time('mysql'));
        } else {
            // Add to leads list
    ...
    {
        if (class_exists('JPSM_Data_Layer')) {
            return JPSM_Data_Layer::get_leads();
        }
    ...
    public static function calculate_tier_from_sales($email)
    {
        ...
        $log = class_exists('JPSM_Data_Layer')
            ? JPSM_Data_Layer::get_sales_by_email($email)
    ...
    {
        if (class_exists('JPSM_Domain_Model')) {
            return JPSM_Domain_Model::get_package_tier($package_name);
        }
    ...
    public static function can_access_folder($email, $folder_path)
    {
        ...
        // Debug Log
        error_log(sprintf(
            "[JPSM Access] User: %s (Tier %d) | Folder: %s | Allowed: [%s] | Access: %s",
            $email,
            $user_tier,
            $folder_path,
            implode(',', $allowed),
    ...
    public static function get_folder_allowed_tiers($folder_path)
    {
        ...
        if (isset($perms[$folder_path]) && is_array($perms[$folder_path])) {
            return $perms[$folder_path];
        }

        // Backward compatibility: Convert old int to array [int, int+1...5] (Hierarchy fallback)
        if (isset($perms[$folder_path]) && is_numeric($perms[$folder_path])) {
            $min_tier = intval($perms[$folder_path]);
            $allowed = [];
        ...
        while (count($parts) > 0) {
            ...
            // 1. Check for ARRAY permissions (Modern)
            if (isset($perms[$test_path]) && is_array($perms[$test_path])) {
                return $perms[$test_path];
            }
            if (isset($perms[$test_path . '/']) && is_array($perms[$test_path . '/'])) {
                return $perms[$test_path . '/'];
            }

            // 2. Check for NUMERIC permissions (Legacy Parent)
            $legacy_val = null;
            if (isset($perms[$test_path]) && is_numeric($perms[$test_path])) {
                $legacy_val = intval($perms[$test_path]);
            } elseif (isset($perms[$test_path . '/']) && is_numeric($perms[$test_path . '/'])) {
                $legacy_val = intval($perms[$test_path . '/']);
            }

            if ($legacy_val !== null) {
                // If min is 2, implies 2, 3, 4, 5
                $allowed = [];
                for ($i = $legacy_val; $i <= self::TIER_FULL; $i++) {
                    $allowed[] = $i;
                }
                return $allowed;
            }
    ...
    public static function set_folder_tier($folder_path, $tier)
    {
        ...
        $allowed = [];
        for ($i = $tier; $i <= self::TIER_FULL; $i++) {
            $allowed[] = $i;
        }
        return self::set_folder_allowed_tiers($folder_path, $allowed);
    }
    ...
    {
        return get_option(self::OPT_FOLDER_PERMS, []);
    }

    /**
     * Demo playback tracking.
     */
    public static function get_play_count($email)
    {
        if (class_exists('JPSM_Data_Layer')) {
            return JPSM_Data_Layer::get_play_count($email);
        }
    ...
    {
        if (class_exists('JPSM_Data_Layer')) {
            return JPSM_Data_Layer::increment_play_count($email);
        }
    ...
    public static function get_current_email()
    {
        if (class_exists('JPSM_Auth')) {
            return JPSM_Auth::get_current_user_email();
        }
        return null;
    }
    ...
    public static function set_access_cookie($email)
    {
        ...
        if (class_exists('JPSM_Auth')) {
            return JPSM_Auth::set_user_session_cookie($email);
        }
    ...
    public static function is_admin($email)
    {
        ...
        $admin_emails = get_option('jpsm_admin_emails', ['jetpackstore.oficial@gmail.com']);
    ...
    public static function verify_session()
    {
        ...
        if (class_exists('JPSM_Auth') && JPSM_Auth::verify_admin_session()) {
            return true;
        }

        // Signed user session that belongs to configured admin emails
        $email = self::get_current_email();
        if ($email && self::is_admin($email)) {
            return true;
        }
    ...
    {
        if (!class_exists('JPSM_Auth') || !JPSM_Auth::validate_request_nonce(array('jpsm_nonce', 'jpsm_login_nonce'))) {
            JPSM_API_Contract::send_error('Invalid nonce', 'invalid_nonce', 401);
        }

        $key = isset($_POST['key']) ? sanitize_text_field(wp_unslash($_POST['key'])) : '';
        $validation = JPSM_API_Contract::validate_required_fields(array('key' => $key));
        if (is_wp_error($validation)) {
            JPSM_API_Contract::send_wp_error($validation, 'Clave requerida', 'missing_required_fields', 422);
        }

        if (JPSM_Auth::set_admin_session_from_key($key)) {
            JPSM_API_Contract::send_success('Bienvenido', 'login_success', 'Sesión iniciada.', 200);
        } else {
            JPSM_API_Contract::send_error('Clave incorrecta', 'invalid_access_key', 401);
        }
    }
    ...
    {
        if (!class_exists('JPSM_Auth') || !JPSM_Auth::validate_request_nonce(array('jpsm_nonce', 'jpsm_logout_nonce'))) {
            JPSM_API_Contract::send_error('Invalid nonce', 'invalid_nonce', 401);
        }

        JPSM_Auth::clear_admin_session_cookie();
        JPSM_API_Contract::send_success('Bye', 'logout_success', 'Sesión cerrada.', 200);
    }

    /**
     * Get user tier information by email
     */
    public static function get_user_tier_ajax()
    {
        ...
        $validation = JPSM_API_Contract::validate_required_fields(array('email' => $email));
        if (is_wp_error($validation)) {
            JPSM_API_Contract::send_wp_error($validation, 'Email requerido', 'missing_required_fields', 422);
        }
    ...
    public static function update_user_tier_ajax()
    {
        ...
        $validation = JPSM_API_Contract::validate_required_fields(array('email' => $email));
        if (is_wp_error($validation)) {
            JPSM_API_Contract::send_wp_error($validation, 'Email inválido', 'missing_required_fields', 422);
        }
        if ($tier < self::TIER_DEMO || $tier > self::TIER_FULL) {
            JPSM_API_Contract::send_error('Tier fuera de rango permitido.', 'invalid_tier', 422, array(
                'min' => self::TIER_DEMO,
                'max' => self::TIER_FULL,
                'provided' => $tier,
            ));
        ...
        if ($res) {
            JPSM_API_Contract::send_success(['saved' => true, 'new_value' => $tier], 'tier_updated', 'Nivel actualizado.', 200);
        } else {
            JPSM_API_Contract::send_error('Error guardando en BD', 'tier_update_failed', 500);
        }
    }
    ...
    public static function log_play_ajax()
    {
        ...
        if (!$email) {
            JPSM_API_Contract::send_error('Email inválido', 'invalid_email', 422);
        }

        $res = self::log_play($email);
        JPSM_API_Contract::send_success($res, 'play_logged', 'Reproducción registrada.', 200);
    }

    /**
     * Get all leads (Demo users)
     */
    public static function get_leads_ajax()
    {
        ...
        JPSM_API_Contract::send_success($leads, 'leads_fetched', 'Prospectos cargados.', 200);
    }

    /**
     * Update folder tier requirement (Matrix Support)
     */
    public static function update_folder_tier_ajax()
    {
        ...
        // 'tiers' can be an array of values
        $tiers = isset($_POST['tiers']) ? $_POST['tiers'] : [];
        $tier_single = isset($_POST['tier']) ? intval($_POST['tier']) : null;

        $validation = JPSM_API_Contract::validate_required_fields(array('folder' => $folder));
        if (is_wp_error($validation)) {
            JPSM_API_Contract::send_wp_error($validation, 'Carpeta requerida', 'missing_required_fields', 422);
        }

        // Support both legacy 'tier' and matrix 'tiers'
        if (is_null($tiers) || $tiers === '' || $tiers === array()) {
            if (!is_null($tier_single) && $tier_single >= 0) {
                $tiers = array($tier_single);
            }
        }

        // If single tier passed (legacy), convert to array
        if (!is_array($tiers) && is_numeric($tiers)) {
            $tiers = [intval($tiers)];
        }
        ...
        if (empty($tiers)) {
            JPSM_API_Contract::send_error('Tiers inválidos.', 'invalid_tier', 422);
        }

        self::set_folder_allowed_tiers($folder, $tiers);
        JPSM_API_Contract::send_success(['message' => 'Permisos actualizados'], 'folder_permissions_updated', 'Permisos actualizados.', 200);
    }

    /**
     * Get all folder permissions (merged with actual folders from Index)
     */
    public static function get_folders_ajax()
    {
        ...
        $db_folders_raw = [];
        if (class_exists('JPSM_Index_Manager')) {
            $table = JPSM_Index_Manager::get_table_name();
            // Get unique folders
        ...
        ksort($final_list);
        JPSM_API_Contract::send_success($final_list, 'folders_fetched', 'Carpetas cargadas.', 200);
    }
}

...
class JPSM_MediaVault_UI
{
    ...
    private static function get_cached_folder_structure($folder, $s3, $ttl = 300)
    {
        ...
        if (!is_wp_error($data) && is_array($data) && count($data) < 500) {
    ...
    public static function handle_ajax()
    {
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'mv_list_folder') {
            ...
            $target_folder = isset($_GET['folder']) ? sanitize_text_field($_GET['folder']) : '';
            ...
            // --- SECURITY CHECK ---
            // Verify if the current user (by Tier) is allowed to access this folder
            ...
            if ($user_tier > 0 && !JPSM_Access_Manager::user_can_access($target_folder, $user_tier)) {
                error_log("[MediaVault] Access Denied for $current_email (Tier $user_tier) to folder: $target_folder");
                JPSM_API_Contract::send_error('⛔ Acceso Denegado: No tienes permiso para ver esta carpeta.');
            }
            // ----------------------
            ...
            foreach ($files_list as $f) {
                $response[] = [
                    'name' => $f['name'],
                    'path' => $f['path'],
                    'url' => $s3->get_presigned_url($f['path'])
                ];
            }
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'mv_search_global') {
            try {
                ...
                error_log("[MediaVault] Global search for: " . $query);
                $results = JPSM_Index_Manager::search($query, $type_filter, 100);
                error_log("[MediaVault] Global search results found: " . count($results));

                // Now generate presigned URLs for results (only for what we found)
                $s3 = new JPSM_S3_Client(
                    JPSM_B2_KEY_ID,
                    JPSM_B2_APP_KEY,
                    JPSM_B2_REGION,
                ...
                foreach ($results as $r) {
                    // Check access for the folder of this file
                    ...
                    // Global Access Check (Enforced for Tier > 0 only, consistent with mv_list_folder)
                    // Demo users (Tier 0) are allowed to see everything (but not download)
                    if ($user_tier > 0 && !JPSM_Access_Manager::user_can_access($folder_path, $user_tier)) {
                        continue; // Skip restricted files for paid users
                    }
                    ...
                    $response[] = [
                        ...
                        'url' => $s3->get_presigned_url($r['path'])
                    ];
                }
        ...
        // ========== LAZY PRESIGNED URL ENDPOINT (Performance Optimization) ==========
        // Generates presigned URLs on-demand instead of during folder listing
        if (isset($_GET['action']) && $_GET['action'] === 'mv_get_presigned_url') {
            ...
            if (empty($path)) {
                JPSM_API_Contract::send_error('Path requerido');
            }
            ...
            if (!$presigned_url) {
                JPSM_API_Contract::send_error('Error generando URL');
            }
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'mv_get_user_meta') {
            ...
            $current_email = JPSM_Access_Manager::get_current_email();
            if (!$current_email || !JPSM_Access_Manager::is_admin($current_email)) {
                JPSM_API_Contract::send_error('No autorizado');
            }
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'mv_update_folder') {
            ...
            $folder_path = isset($_POST['folder']) ? sanitize_text_field($_POST['folder']) : '';
    ...
    public static function render()
    {
        ...
        if (isset($_GET['invitado']) && $_GET['invitado'] == '1') {
            ...
            // Redirect to clean URL to avoid re-triggering and load the vault
            wp_redirect(remove_query_arg('invitado'));
            exit;
        }
        // --------------------------------
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'mv_logout') {
            ...
            wp_redirect(remove_query_arg(['action', 'folder']));
            exit;
        }
        ...
        while ($depth_safety < 10) {
            $check = self::get_cached_folder_structure($drill_path, $s3, 300); // 5 min cache
        ...
        // --- STEP 2: Initial Redirect (Bypass Junction View) ---
        if (empty($current_folder)) {
            $junction_check = self::get_cached_folder_structure($junction_folder, $s3, 300);
            if (!is_wp_error($junction_check) && !empty($junction_check['folders'])) {
                // Drop into the first category immediately
                $current_folder = $junction_check['folders'][0];
            } else {
                $current_folder = $junction_folder;
            }
        }

        // --- STEP 3: Breadcrumb Data Pruning (for initial load) ---
        $data = self::get_cached_folder_structure($current_folder, $s3);
        ...
        if (isset($_GET['mv_ajax']) && $_GET['mv_ajax'] == '1') {
            ...
            // Calculate Depth for JS logic
            $current_depth = count(array_filter(explode('/', $current_folder)));
            ...
            foreach ($data['files'] as $file) {
                ...
                $presigned_url = $s3->get_presigned_url($file['path']);
        ...
        $sidebar_content = self::get_cached_folder_structure($junction_folder, $s3, 300);
                ...
                <nav class="mv-sidebar-nav">
                    ...
                    <?php if (!empty($sidebar_folders)): ?>
                        ...
                        <?php foreach ($sidebar_folders as $s_folder): ?>
                            <?php
                            // Access check removed for Funnel Mode (Sidebar Visibility)
                            // Only check if NOT Demo (Tier > 0)
                            if ($user_tier > 0 && !JPSM_Access_Manager::can_access_folder($user_email, $s_folder)) {
                                continue;
                            }
                            ...
                            $is_active = ($current_folder === $s_folder) || (strpos($current_folder, $s_folder) === 0);
                            ?>
                ...
                <div class="mv-sidebar-footer">
                    <div style="font-size: 0.85rem; color: var(--mv-text-muted); margin-bottom: 8px;">
                        ...
                        <span
                            ...
                            <?php echo JPSM_Access_Manager::get_tier_name($user_tier); ?>
                        </span>
                    </div>
                    <?php if ($user_tier == 0): ?>
            ...
            <div class="mv-main-content">
                ...
                if ($tier_banner === 0):
                    ...
                    ">
                        ...
                        <a href="https://wa.me/521234567890?text=Hola,%20quiero%20actualizar%20mi%20Plan%20MediaVault"
                            target="_blank" style="
                            background: white;
                            color: #d97706;
                            padding: 8px 16px;
                ...
                <div class="jpsm-mv-grid view-list <?php echo $force_list_view ? 'force-list-view' : ''; ?>" id="mv-grid">
                    ...
                    <?php foreach ($data['files'] as $file): ?>
                        ...
                        <div class="jpsm-mv-card mv-item-file" data-name="<?php echo esc_attr(strtolower($file['name'])); ?>"
                            ...
                            data-type="file" data-path="<?php echo esc_attr($file['path']); ?>">
                            ...
                            <div style="flex:1;">
                                ...
                                <div class="jpsm-mv-meta"><?php echo strtoupper($ext); ?> •
                            ...
                            <div style="display:flex; gap:8px; margin-top:12px; padding: 0 16px;">
                                <?php if (in_array(strtolower($ext), ['mp3', 'wav', 'mp4', 'mov'])): ?>
                                    <button type="button" class="mv-preview-btn" data-url="<?php echo esc_url($presigned_url); ?>"
                                ...
                                <?php
                                $tier_check_file = JPSM_Access_Manager::get_user_tier(JPSM_Access_Manager::get_current_email());
                                if ($tier_check_file > 0):
                                    ?>
                                    <a href="<?php echo esc_url($presigned_url); ?>" class="mv-download-btn"
        ...
        $current_email = JPSM_Access_Manager::get_current_email();
        $user_tier = $current_email ? JPSM_Access_Manager::get_user_tier($current_email) : 0;
        $is_admin = $current_email && JPSM_Access_Manager::is_admin($current_email);
        $remaining_plays = $current_email ? JPSM_Access_Manager::get_remaining_plays($current_email) : 0;
        ?>
        <script>      window.MV_USER_DATA = {
                                        email: <?php echo json_encode($current_email); ?>,
                                        tier: <?php echo intval($user_tier); ?>,
                                        tierName: <?php echo json_encode(JPSM_Access_Manager::get_tier_name($user_tier)); ?>,
                                        isAdmin: <?php echo $is_admin ? 'true' : 'false'; ?>,
                                        remainingPlays: <?php echo intval($remaining_plays); ?>,
                                        whatsappNumber: '525568755277',
                                        folderPerms: <?php echo json_encode(JPSM_Access_Manager::get_all_folder_permissions()); ?>,
                                        ajax_url: '<?php echo admin_url('admin-ajax.php'); ?>',
                                        nonce: '<?php echo esc_js(wp_create_nonce('jpsm_mediavault_nonce')); ?>',
                                        access_nonce: '<?php echo esc_js(wp_create_nonce('jpsm_access_nonce')); ?>',
                                        sidebarFolders: <?php echo json_encode(array_values($sidebar_folders)); ?>
                                    };
                                </script>
        ...
        ?>
                                        ...
                                        <form method="post">
                                            <div style="margin-bottom:24px;">
                                                <label style="display:block; color:#334155; font-weight:600; font-size:14px; margin-bottom:8px;">Correo
                                                    Electrónico</label>
                                                <input type="email" name="jdd_email" placeholder="tucorreo@ejemplo.com" required
                                ...
                                if (isset($_POST['jdd_login']) && isset($_POST['jdd_email'])) {
                                    ...
                                    if (!empty($email) && is_email($email)) {
                                        JPSM_Access_Manager::set_access_cookie($email);
                                        echo "<script>window.location.reload();</script>";
                                    } else {
                                        echo "<p style='color:#ef4444; text-align:center; margin-top:16px; font-weight:500;'>Por favor ingresa un correo válido.</p>";
                                    }
                                }
                                return ob_get_clean();
    }

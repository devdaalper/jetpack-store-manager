...
class JPSM_S3_Client
{
    ...
    public function list_objects($prefix = '')
    {
        ...
        do {
            ...
            if ($code !== 200) {
                return new WP_Error('b2_error', "B2 error {$code}: {$body}");
            }
     ...
     */
    public function get_presigned_url($file_path, $expires_in = 86400)
    {
        $method = 'GET';
        $service = 's3';
    ...
    public function list_objects_recursive($prefix)
    {
        ...
        do {
            ...
            $code = wp_remote_retrieve_response_code($response);
            if ($code !== 200) {
                return new WP_Error('s3_error', 'S3 Error ' . $code);
            }
     ...
     */
    public function list_objects_page($prefix, $continuation_token = null)
    {
        $files = [];
        ...
        $query_params = [
            'list-type' => '2',
            'prefix' => $prefix,
            'max-keys' => '1000' // Process 1000 files per batch
        ];

"""BPM detection engine combining text-pattern, ID3-metadata, and acoustic analysis."""
...
@dataclass
class BPMEstimate:
    """A single BPM estimate with source, confidence, and optional notes."""
...
def parse_bpm_number(raw: str) -> int:
    """Extract and validate a BPM integer from a raw string, returning 0 if invalid."""
...
def detect_bpm_from_text(path: str) -> Optional[BPMEstimate]:
    """Attempt to detect BPM from patterns in the file path or name."""
    text = str(path).lower()
...
def extract_bpm_from_mp3_head(head_bytes: bytes) -> Optional[BPMEstimate]:
    """Parse the ID3v2 TBPM frame from raw MP3 header bytes."""
...
def normalize_half_double(bpm: float) -> float:
    """Normalize BPM into the 70-180 range by halving or doubling."""
    if bpm <= 0:
        return 0.0
    while bpm < 70:
        bpm *= 2
...
def estimate_bpm_acoustic(ffmpeg_path: str, url: str, auth_token: str, sample_seconds: int) -> Optional[BPMEstimate]:
    """Run acoustic BPM detection using librosa and aubio on audio decoded via FFmpeg."""

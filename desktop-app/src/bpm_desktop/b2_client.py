"""Backblaze B2 API client for authorization, bucket listing, and audio file retrieval."""
...
class B2Error(RuntimeError):
    """Raised when a B2 API call fails or returns an unexpected response."""


@dataclass
class B2AuthSession:
    """Holds tokens and URLs obtained from B2 authorization."""
    account_id: str
    api_url: str
    download_url: str
...
class B2Client:
    """Client for interacting with the Backblaze B2 cloud storage API."""
    ...
    def authorize(self) -> B2AuthSession:
        """Authenticate with B2 and store the session tokens."""
        ...
        logger.info("B2 authorized (account=%s)", self.session.account_id)
        return self.session

    def ensure_bucket_id(self) -> str:
        """Resolve and cache the bucket ID for the configured bucket name."""
        if self.bucket_id:
        ...
        logger.debug("Resolved bucket '%s' → %s", self.bucket_name, self.bucket_id)
        return self.bucket_id

    def list_audio_page(self, start_file_name: str | None = None, max_file_count: int = 1000) -> tuple[list[CatalogObject], str | None]:
        """Fetch one page of audio files from B2, returning objects and the next cursor."""
        auth = self.session or self.authorize()
    ...
    def iter_audio_objects(self, max_pages: int = 200) -> Iterable[CatalogObject]:
        """Iterate over all audio objects in the bucket, paginating automatically."""
        start = None
        pages = 0
    ...
    def build_download_url(self, path: str) -> str:
        """Build the full download URL for a file in the bucket."""
        auth = self.session or self.authorize()
    ...
    def fetch_head_bytes(self, path: str, max_bytes: int = 2048) -> bytes:
        """Download the first N bytes of a file for metadata inspection."""
        auth = self.session or self.authorize()

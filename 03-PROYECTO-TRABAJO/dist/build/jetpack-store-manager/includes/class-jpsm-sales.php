...
class JPSM_Sales
{
    ...
    public static function process_sale_ajax()
    {
        ...
        if (class_exists('JPSM_Access_Manager') && is_email($email)) {
            $tier_after = JPSM_Access_Manager::get_user_tier($email);
            if ($resolved_tier > 0 && $tier_after < $resolved_tier) {
                JPSM_Access_Manager::set_user_tier($email, $resolved_tier);
            }
        }

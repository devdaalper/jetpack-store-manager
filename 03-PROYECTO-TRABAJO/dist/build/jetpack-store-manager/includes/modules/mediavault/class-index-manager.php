...
class JPSM_Index_Manager
{
    ...
    private static $has_index_data_cache = null;
     ...
     * Uses a cheap existence query instead of COUNT(*).
     */
    public static function has_index_data()
    {
        if (self::$has_index_data_cache !== null) {
            return self::$has_index_data_cache;
        }
        ...
        self::$has_index_data_cache = $has;
        return $has;
    }

    /**
     * List a folder's immediate children using the local index.
     *
     * Returns the same shape as JPSM_S3_Client::list_objects():
     * - folders: string[] (prefixes ending in '/')
     * - files: array[] of { name, path, size, date }
     */
    public static function list_folder_structure($folder)
    {
        ...
        // Index stores 'folder' as dirname(path) without trailing slash (e.g. "a/b").
        // Use that to keep the SQL portable (SQLite/MySQL) and fast.
        $folder_key = rtrim($prefix, '/');

        // 1) Direct files in this folder.
        if ($folder_key === '') {
            // Root files: dirname('file.ext') becomes '.' in sync code.
            $files_sql = "SELECT path, name, size, synced_at
                          FROM $table_name
                          WHERE folder IN ('.','')
                          ORDER BY name ASC";
            ...
            $files_sql = $wpdb->prepare(
                "SELECT path, name, size, synced_at
                 FROM $table_name
                 WHERE folder = %s
                 ORDER BY name ASC",
                $folder_key
        ...
        if (is_array($files_rows)) {
            foreach ($files_rows as $r) {
                $files[] = [
                    ...
                    'path' => (string) ($r['path'] ?? ''),
                    'size' => (int) ($r['size'] ?? 0),
                    // Index doesn't store LastModified; use synced_at as a lightweight hint.
                    'date' => (string) ($r['synced_at'] ?? ''),
                ];
            }
        }

        // 2) Immediate subfolders (derived from distinct 'folder' values under this prefix).
        if ($folder_key === '') {
        ...
        if (is_array($folders_rows)) {
            foreach ($folders_rows as $f) {
                ...
                if ($folder_key === '') {
                    // Top-level child is the first segment of the folder path.
                    $parts = explode('/', $f);
                    $child = trim((string) ($parts[0] ?? ''));
                } else {
                    // Child is first segment after "<folder_key>/".

...
class MediaVaultApp {
    ...
    openUpgradeDialog(targetName, allowedTiers) {
        ...
        const adminEmail = (this.userData.adminEmail || '').toString().trim();
        ...
        if (adminEmail) {
            const subject = encodeURIComponent('Solicitud de upgrade');
            window.open(`mailto:${adminEmail}?subject=${subject}&body=${encodeURIComponent(message)}`, '_blank');
            return;
        }

        alert('Este sitio no tiene configurado un contacto de upgrade (WhatsApp o email).');
    }
    ...
    filterCards(query) {
        ...
        document.querySelectorAll('.jpsm-mv-card').forEach(card => {
            ...
            if (this.currentFilter !== 'all' && typeRaw === 'folder' && !allowFoldersInShallowLevels) {
                typeMatch = false;
            }
    ...
    async performGlobalSearch(query) {
        ...
        try {
            ...
            if (data.success) {
                if (grid) {
                    ...
                    if (filesList.length > 0) {
                        ...
                        filesList.forEach(file => {
                            ...
                            const html = `
                            <div class="jpsm-mv-card ${lockedClass}" data-name="${safeName}" data-type="file" data-path="${safePath}">
                                <div class="jpsm-mv-cover" style="display:none !important;"></div>
                ...
                if (grid) grid.innerHTML = '<div style="grid-column: 1/-1; text-align:center; color:red;">Error: ' + mvEscapeHtml(data.data) + '</div>';
            }
        } catch (err) {
    ...
    renderGrid(data) {
        ...
        data.files.forEach(f => {
            ...
            if (f && f.can_download) {
	            ...
	            const html = `
	                <div class="jpsm-mv-card mv-item-file" data-name="${mvEscapeAttr(String(f.name || '').toLowerCase())}" data-date="${mvEscapeAttr(f.date)}" data-size="${mvEscapeAttr(f.size)}" data-type="file" data-path="${safePath}">
	                    ...
	                    <div style="flex:1;">
	                        ...
	                        <div class="jpsm-mv-meta">${mvEscapeHtml(f.size_fmt)} • ${mvEscapeHtml(f.date)}</div>
...
class AdminManager {
    ...
    openWhatsApp(folderName) {
        ...
        const adminEmail = (this.userData.adminEmail || '').toString().trim();
        if (adminEmail) {
            const subject = encodeURIComponent('Solicitud de acceso');
            window.open(`mailto:${adminEmail}?subject=${subject}&body=${encodeURIComponent(message)}`, '_blank');
            return;
        }

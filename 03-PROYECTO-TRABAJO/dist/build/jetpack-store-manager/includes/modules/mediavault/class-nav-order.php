...
class JPSM_MediaVault_Nav_Order
{
    ...
    public static function list_folder_structure($folder, $s3 = null)
    {
        ...
        if (
            class_exists('JPSM_Index_Manager')
            && method_exists('JPSM_Index_Manager', 'has_index_data')
            && method_exists('JPSM_Index_Manager', 'list_folder_structure')
            && JPSM_Index_Manager::has_index_data()
        ) {
    ...
    public static function ajax_get_sidebar_folders()
    {
        ...
        if (is_wp_error($auth)) {
            JPSM_API_Contract::send_wp_error($auth, 'No autorizado', 'unauthorized', 403);
        }

        $res = self::get_available_sidebar_folders();
        if (is_wp_error($res)) {
            JPSM_API_Contract::send_wp_error($res, 'Error obteniendo carpetas', 'mv_nav_folders_error', 500);
        }

        JPSM_API_Contract::send_success($res, 'mv_nav_folders_ok', 'OK');
    }
    ...
    public static function ajax_save_sidebar_order()
    {
        ...
        if (is_wp_error($available)) {
            JPSM_API_Contract::send_wp_error($available, 'No se pudo validar la lista de carpetas', 'mv_nav_validate_error', 500);
        }
    ...
    public static function ajax_reset_sidebar_order()
    {
        ...
        JPSM_API_Contract::send_success($res, 'mv_nav_order_reset', 'OK');
    }
}

 ...
 * Separated from Admin to keep the controller clean.
 */
class Admin_Views
{
    public static function render_setup_page()
    {
        ...
        $mv_page_id = intval(get_option(Admin::OPTION_MEDIAVAULT_PAGE_ID, 0));
        $mgr_page_id = intval(get_option(Admin::OPTION_MANAGER_PAGE_ID, 0));
        $mv_page_ok = ($mv_page_id > 0 && get_post_status($mv_page_id));
        $mgr_page_ok = ($mgr_page_id > 0 && get_post_status($mgr_page_id));

        $b2_ok = class_exists(Config::class) ? Config::is_b2_configured() : false;
        $b2_missing = class_exists(Config::class) ? Config::get_b2_missing_fields() : array('jpsm_b2_key_id', 'jpsm_b2_app_key', 'jpsm_b2_bucket', 'jpsm_b2_region');

        $index_stats = class_exists(Index_Manager::class) ? Index_Manager::get_stats() : array();
                ...
                <table class="widefat striped" style="max-width: 980px;">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php
                                $stale = class_exists(Index_Manager::class) ? Index_Manager::is_stale() : true;
    ...
    {
        $sale_package_options = class_exists(Domain_Model::class)
            ? Domain_Model::get_sale_package_options()
            : array(
                array('id' => 'basic', 'label' => 'Básico', 'icon' => ''),
                array('id' => 'vip', 'label' => 'VIP', 'icon' => ''),
                array('id' => 'full', 'label' => 'Full', 'icon' => ''),
            );
        $vip_variant_options = class_exists(Domain_Model::class)
            ? Domain_Model::get_vip_variant_options()
            : array(
    ...
    {
        $overview = class_exists(Finance::class) ? Finance::get_admin_overview() : array();
                                            ...
                                            <?php if (!empty($sales_picker_rows)): ?>
                                                <?php foreach ($sales_picker_rows as $row): ?>
                                                    <tr>
                                                        ...
                                                        <td><?php echo esc_html(class_exists(Finance::class) ? Finance::market_label($row['market']) : strtoupper((string) $row['market'])); ?></td>
                                ...
                                <?php if (!empty($recent_settlements)): ?>
                                    <?php foreach ($recent_settlements as $settlement): ?>
                                        <tr>
                                            ...
                                            <td><?php echo esc_html(class_exists(Finance::class) ? Finance::market_label($settlement['market'] ?? '') : (string) ($settlement['market'] ?? '')); ?></td>
                                            <td><?php echo esc_html(class_exists(Finance::class) ? Finance::channel_label($settlement['channel'] ?? '') : (string) ($settlement['channel'] ?? '')); ?></td>
                                ...
                                <?php if (!empty($recent_expenses)): ?>
                                    <?php foreach ($recent_expenses as $expense): ?>
                                        <tr>
                                            ...
                                            <td><?php echo esc_html(class_exists(Finance::class) ? Finance::expense_category_label($expense['category'] ?? '') : (string) ($expense['category'] ?? '')); ?></td>
    ...
    {
        $settings_packages = class_exists(Domain_Model::class)
            ? Domain_Model::get_settings_packages()
            : array(
                array(
                    'id' => 'basic',
                    'label' => 'Básico',
        ...
        <hr>
	        ...
	        <?php if (class_exists(Config::class) && !Config::is_b2_configured()): ?>
	            <?php $missing = Config::get_b2_missing_fields(); ?>
                ...
                <table class="form-table">
                    <tr valign="top">
                        ...
                        <td>
                            ...
                            ?>
                            <select name="<?php echo esc_attr(Admin::OPTION_MEDIAVAULT_PAGE_ID); ?>">
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            <?php $mgr_page_id = intval(get_option(Admin::OPTION_MANAGER_PAGE_ID, 0)); ?>
                            <select name="<?php echo esc_attr(Admin::OPTION_MANAGER_PAGE_ID); ?>">
...
class_alias(Admin_Views::class, 'JPSM_Admin_Views');

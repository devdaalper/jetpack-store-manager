...
class Behavior_Service
{
    ...
    public static function track_behavior_event_ajax()
    {
        ...
        if (is_wp_error($auth)) {
            ...
            API_Contract::send_wp_error($auth, 'No autorizado', 'unauthorized', $status);
        }

        $event = self::build_event_payload($_REQUEST);
        if (is_wp_error($event)) {
            API_Contract::send_wp_error($event, 'Evento inválido', 'invalid_event', 422);
        }

        $result = class_exists(Data_Layer::class)
            ? Data_Layer::insert_behavior_event($event)
    ...
    public static function track_event_passive($event_name, $fields = array())
    {
        try {
            ...
            if (is_wp_error($event) || !class_exists(Data_Layer::class)) {
                return false;
            }

            $result = Data_Layer::insert_behavior_event($event);
    ...
    public static function get_behavior_report_ajax()
    {
        ...
        if (is_wp_error($report)) {
            API_Contract::send_wp_error($report, 'No se pudo generar reporte', 'behavior_report_error', 500);
        }

        API_Contract::send_success($report, 'behavior_report_fetched', 'Reporte cargado.', 200);
    }
    ...
    public static function get_transfer_report_ajax()
    {
        ...
        if (is_wp_error($report)) {
            API_Contract::send_wp_error($report, 'No se pudo generar reporte', 'transfer_report_error', 500);
        }

        API_Contract::send_success($report, 'transfer_report_fetched', 'Reporte de transferencia cargado.', 200);
    }
    ...
    public static function export_behavior_csv_ajax()
    {
        ...
        $report = self::build_behavior_report($month, $filters);
        if (is_wp_error($report)) {
            API_Contract::send_wp_error($report, 'No se pudo generar CSV', 'behavior_export_error', 500);
        }
    ...
    public static function export_transfer_csv_ajax()
    {
        ...
        $report = self::build_transfer_report($month, $filters, $window);
        if (is_wp_error($report)) {
            API_Contract::send_wp_error($report, 'No se pudo generar CSV', 'transfer_export_error', 500);
        }
    ...
    public static function run_daily_rollup()
    {
        ...
        Data_Layer::rebuild_behavior_daily($from, $today);
    }
    ...
    private static function build_event_payload($source)
    {
        ...
        if (class_exists(Access_Manager::class)) {
            $email = (string) Access_Manager::get_current_email();
        }

        $session_cookie_name = class_exists(Auth::class) ? Auth::USER_SESSION_COOKIE : 'jdd_access_token';
        ...
        if ($tier === null || $tier === '' || $tier === 'all') {
            $tier = 0;
            if ($email !== '' && class_exists(Access_Manager::class)) {
                $tier = intval(Access_Manager::get_user_tier($email));
            }
        }
        $tier = intval($tier);
    ...
    private static function resolve_region_for_email($email)
    {
        ...
        if (!is_email($email) || !class_exists(Data_Layer::class)) {
            return 'unknown';
        }
    ...
    private static function maybe_warm_behavior_daily($from, $to)
    {
        ...
        if (!Data_Layer::behavior_daily_has_data($from, $to)) {
            Data_Layer::rebuild_behavior_daily($from, $to);
            return;
        }
        ...
        Data_Layer::rebuild_behavior_daily($rolling_from, $to_date);
    }
    ...
    private static function build_behavior_report($month, $filters)
    {
        ...
        $top_keywords_current = Data_Layer::get_behavior_top_keywords($current['start'], $current['end'], $filters, 30, false);
        $top_keywords_prev = Data_Layer::get_behavior_top_keywords($prev['start'], $prev['end'], $filters, 200, false);
        $top_keywords_yoy = Data_Layer::get_behavior_top_keywords($yoy['start'], $yoy['end'], $filters, 200, false);
        $top_zero_keywords = Data_Layer::get_behavior_top_keywords($current['start'], $current['end'], $filters, 30, true);
        ...
        $top_downloads_current = Data_Layer::get_behavior_top_downloads($current['start'], $current['end'], $filters, 30);
        $top_downloads_prev = Data_Layer::get_behavior_top_downloads($prev['start'], $prev['end'], $filters, 300);
        $top_downloads_yoy = Data_Layer::get_behavior_top_downloads($yoy['start'], $yoy['end'], $filters, 300);
        ...
        $search_total = Data_Layer::get_behavior_metric_sum(array('search_executed'), $current['start'], $current['end'], $filters);
        $zero_total = Data_Layer::get_behavior_metric_sum(array('search_zero_results'), $current['start'], $current['end'], $filters);
        $download_intent_total = Data_Layer::get_behavior_metric_sum(
            array('download_file_click', 'download_folder_click'),
            $current['start'],
            $current['end'],
            $filters
        );
        $download_granted_total = Data_Layer::get_behavior_metric_sum(
            array('download_file_granted', 'download_folder_granted'),
            $current['start'],
            $current['end'],
            $filters
        );
        $download_denied_total = Data_Layer::get_behavior_metric_sum(
        ...
        $search_segments = Data_Layer::get_behavior_segment_preferences(
            $current['start'],
            $current['end'],
            $filters,
        ...
        $download_segments = Data_Layer::get_behavior_segment_preferences(
            $current['start'],
            $current['end'],
            $filters,
            array('download_file_granted', 'download_folder_granted'),
    ...
    private static function build_transfer_report($month, $filters, $window = 'month')
    {
        ...
        Data_Layer::backfill_transfer_authorized_from_folder_events($current['end']);
        ...
        $observed_month = Data_Layer::get_behavior_metric_bytes_sum($transfer_keys, $current['start'], $current['end'], $filters, 'observed');
        $authorized_month = Data_Layer::get_behavior_metric_bytes_sum($transfer_keys, $current['start'], $current['end'], $filters, 'authorized');
        $observed_lifetime = Data_Layer::get_behavior_metric_bytes_sum($transfer_keys, '2000-01-01', $current['end'], $filters, 'observed');
        $authorized_lifetime = Data_Layer::get_behavior_metric_bytes_sum($transfer_keys, '2000-01-01', $current['end'], $filters, 'authorized');

        $registered_month = max(0, intval(Data_Layer::count_registered_users_dedup($series_anchor_month)));
        $registered_lifetime = max(0, intval(Data_Layer::count_registered_users_dedup('')));

        $coverage_counts = Data_Layer::get_behavior_transfer_coverage_counts($current['start'], $current['end'], $filters, $coverage_keys);
        ...
        $daily_rows = Data_Layer::get_behavior_transfer_daily_series($daily_start_90d, $current['end'], $filters, $transfer_keys);
        $series_daily_90d = self::fill_transfer_daily_series($daily_start_90d, $current['end'], $daily_rows);

        $monthly_rows = Data_Layer::get_behavior_transfer_monthly_series($monthly_start, $series_anchor_month, $filters, $transfer_keys);
        ...
        foreach ($series_monthly_absolute as $row) {
            ...
            $month_registered = max(0, intval(Data_Layer::count_registered_users_dedup($row_month)));
            $series_monthly_relative[] = array(
                'month' => $row_month,
        ...
        $lifetime_rows = Data_Layer::get_behavior_transfer_monthly_series('2000-01', $series_anchor_month, $filters, $transfer_keys);
        ...
        $top_current = Data_Layer::get_behavior_transfer_top_folders($current['start'], $current['end'], $filters, 30);
        $top_prev_map = array();
        $top_yoy_map = array();
        if ($supports_monthly_deltas) {
            $top_prev = Data_Layer::get_behavior_transfer_top_folders($prev['start'], $prev['end'], $filters, 300);
            $top_yoy = Data_Layer::get_behavior_transfer_top_folders($yoy['start'], $yoy['end'], $filters, 300);
        ...
        $downloads_total_window = max(0, intval(Data_Layer::get_behavior_metric_sum($folder_download_metric_keys, $current['start'], $current['end'], $filters)));
        $unique_folders_window = max(0, intval(Data_Layer::get_behavior_transfer_unique_folder_count($current['start'], $current['end'], $filters, $folder_download_metric_keys)));
        $legacy_totals = Data_Layer::get_folder_download_totals_by_range($current['start'], $current['end']);
        ...
        if ($needs_fallback) {
            ...
            $legacy_top_current = Data_Layer::get_top_folder_downloads_by_range($current['start'], $current['end'], 30);
            $legacy_prev_map = array();
            $legacy_yoy_map = array();
            if ($supports_monthly_deltas) {
                foreach ((array) Data_Layer::get_top_folder_downloads_by_range($prev['start'], $prev['end'], 300) as $row) {
                    $path = (string) ($row['folder_path'] ?? '');
                    if ($path !== '') {
                        $legacy_prev_map[$path] = max(0, intval($row['downloads'] ?? 0));
                    }
                }
                foreach ((array) Data_Layer::get_top_folder_downloads_by_range($yoy['start'], $yoy['end'], 300) as $row) {
                    $path = (string) ($row['folder_path'] ?? '');
                    if ($path !== '') {
...
class_alias(Behavior_Service::class, 'JPSM_Behavior_Service');

 ...
 * services (Stats_Service / Access_Service) and templates.
 */
class Dashboard
{
    /**
     ...
     *
     * @return string HTML output for the dashboard shortcode.
     */
    public static function render()
    {
        ...
        $stats = array();
        if (class_exists(Stats_Service::class)) {
            $stats = Stats_Service::get_frontend_dashboard_stats();
        }
        if (!is_array($stats)) {
            $stats = array();
        }
        ...
        $auth_method = class_exists(Access_Service::class)
            ? Access_Service::get_dashboard_auth_method()
            : 'none';

        $operator_label = class_exists(Access_Service::class)
            ? Access_Service::get_operator_label()
        ...
        $script_data = array(
            ...
            'behavior_report' => $stats['behavior_report'],
            'transfer_report' => $stats['transfer_report'],
        );
    ...
    private static function verify_dashboard_access()
    {
        if (class_exists(Access_Service::class)) {
            return Access_Service::can_access_dashboard();
        }
        ...
        if (class_exists(Access_Manager::class)) {
            return Access_Manager::verify_session();
        }
    ...
    private static function get_inline_style_fallback()
    {
        ...
        if (!did_action('wp_head') && defined('JPSM_PLUGIN_URL')) {
            ...
            wp_enqueue_style('jpsm-admin-css', JPSM_PLUGIN_URL . $css_path, array(), JPSM_VERSION, 'all');
            return '';
        }
    ...
    private static function enqueue_dashboard_assets()
    {
        ...
        if (!wp_script_is('chart-js', 'registered')) {
            wp_register_script('chart-js', 'https://cdn.jsdelivr.net/npm/chart.js@3.9.1', array(), '3.9.1', true);
        }
        wp_enqueue_script('chart-js');

        $base_dir = defined('JPSM_PLUGIN_DIR') ? JPSM_PLUGIN_DIR : plugin_dir_path(__FILE__) . '../';
        $app_path = jpsm_asset_path('assets/js/jpsm-app.js');
        $charts_path = jpsm_asset_path('assets/js/jpsm-dashboard-charts.js');
        $app_file = $base_dir . $app_path;
        $charts_file = $base_dir . $charts_path;
        $app_version = file_exists($app_file) ? strval(filemtime($app_file)) : JPSM_VERSION;
        $charts_version = file_exists($charts_file) ? strval(filemtime($charts_file)) : JPSM_VERSION;

        $app_url = plugins_url('../' . $app_path, __FILE__);
        wp_enqueue_script('jpsm-app', $app_url, array('chart-js'), $app_version, true);

        $charts_url = plugins_url('../' . $charts_path, __FILE__);
        wp_enqueue_script('jpsm-dashboard-charts', $charts_url, array('jpsm-app', 'chart-js'), $charts_version, true);
    }

    private static function get_sale_package_options()
    {
        if (class_exists(Domain_Model::class)) {
            return Domain_Model::get_sale_package_options();
        }
    ...
    {
        if (class_exists(Domain_Model::class)) {
            return Domain_Model::get_vip_variant_options();
        }
    ...
    private static function render_login_form()
    {
        ...
        $login_path = jpsm_asset_path('assets/js/jpsm-dashboard-login.js');
        $login_js_url = plugins_url('../' . $login_path, __FILE__);
...
class_alias(Dashboard::class, 'JPSM_Dashboard');

...
/**
 * Finance module for settlements, expenses, and financial reporting.
 *
 * Provides settlement recording, expense tracking, financial overview
 * aggregation, and related AJAX endpoints.
 *
...
class Finance
{
    ...
    /**
     * Get available market options for settlement forms.
     *
     * @return array[] List of market option arrays with id, label, and description.
     */
    ...
    /**
     * Get available payment channel options.
     *
     * @return array<string, string> Channel slug => label map.
     */
    ...
    /**
     * Get available expense category options.
     *
     * @return array<string, string> Category slug => label map.
     */
    ...
    /**
     * Build the full admin finance overview including KPIs, sales picker, and recent records.
     *
     * @return array Overview data with current_month, lifetime, sales_picker_rows, recent records, and options.
     */
    public static function get_admin_overview()
    {
        $sales_log = class_exists(Data_Layer::class) ? Data_Layer::get_sales_log() : array();
        $settlements = class_exists(Data_Layer::class) ? Data_Layer::get_finance_settlements(0) : array();
        $expenses = class_exists(Data_Layer::class) ? Data_Layer::get_finance_expenses(0) : array();
        $linked_sale_uids = class_exists(Data_Layer::class) ? Data_Layer::get_finance_linked_sale_uids() : array();
        ...
        $sales_picker_rows = self::build_sales_picker_rows(
            class_exists(Data_Layer::class) ? Data_Layer::get_sales_log(self::SALES_PICKER_LIMIT) : array_slice((array) $sales_log, 0, self::SALES_PICKER_LIMIT),
            $linked_sale_uids
        );
    ...
    /**
     * Aggregate financial overview from raw sales, settlements, and expenses.
     *
     * @param array $sales_log       Sales log entries.
     * @param array $settlements     Settlement records.
     * @param array $expenses        Expense records.
     * @param array $linked_sale_uids UIDs of sales already linked to settlements.
     * @return array Overview with current_month and lifetime summary buckets.
     */
    ...
    /**
     * AJAX handler to record a new settlement, optionally linking selected sales.
     *
    ...
    public static function record_settlement_ajax()
    {
        ...
        $selected_sales = class_exists(Data_Layer::class) ? Data_Layer::get_sales_by_uids($sale_uids) : array();
        $linked_sale_uids = class_exists(Data_Layer::class) ? Data_Layer::get_finance_linked_sale_uids() : array();

        if (!empty($sale_uids) && count($selected_sales) !== count($sale_uids)) {
            API_Contract::send_error('Una o más ventas seleccionadas ya no existen o no pudieron cargarse.', 'missing_selected_sales', 404);
        }

        foreach ($sale_uids as $sale_uid) {
            if (in_array($sale_uid, $linked_sale_uids, true)) {
                API_Contract::send_error('Una o más ventas seleccionadas ya están conciliadas.', 'sale_already_settled', 409);
            }
        }

        $derived = self::derive_context_from_sales($selected_sales);
        if (is_wp_error($derived)) {
            API_Contract::send_wp_error($derived, 'Las ventas seleccionadas no son compatibles entre sí.', 'invalid_sales_mix', 422);
            return;
        }
        ...
        if ($market === '' || !in_array($market, array('mx', 'us', 'manual'), true)) {
            API_Contract::send_error('Selecciona un mercado válido.', 'invalid_market', 422);
        }

        if ($gross_amount <= 0) {
            API_Contract::send_error('Captura un ingreso bruto válido o selecciona ventas para conciliar.', 'invalid_gross_amount', 422);
        }

        if ($currency === 'USD' && $net_amount_mxn <= 0) {
            API_Contract::send_error('Para USD captura el neto recibido en MXN o el tipo de cambio efectivo.', 'missing_mxn_net', 422);
        }
        ...
        $saved = class_exists(Data_Layer::class) ? Data_Layer::create_finance_settlement($settlement, $items) : false;
        if (!$saved) {
            API_Contract::send_error('No se pudo registrar la liquidación.', 'finance_settlement_save_failed', 500);
        }
    ...
    /**
     * AJAX handler to record a new expense.
     *
     * @return void Sends JSON response and exits.
     */
    public static function record_expense_ajax()
    {
        ...
        if ($amount <= 0) {
            API_Contract::send_error('Captura un monto de gasto válido.', 'invalid_expense_amount', 422);
        }

        if ($currency === 'USD' && $amount_mxn <= 0) {
            API_Contract::send_error('Para gastos en USD captura el monto equivalente en MXN o un tipo de cambio.', 'missing_expense_mxn', 422);
        }
        ...
        if ($category === '') {
            API_Contract::send_error('Selecciona una categoría de gasto.', 'missing_expense_category', 422);
        }
        if ($vendor === '' && $description === '') {
            API_Contract::send_error('Describe el gasto o indica el proveedor.', 'missing_expense_description', 422);
        }
        ...
        $saved = class_exists(Data_Layer::class) ? Data_Layer::create_finance_expense($expense) : false;
        if (!$saved) {
            API_Contract::send_error('No se pudo registrar el gasto.', 'finance_expense_save_failed', 500);
        }
    ...
    /**
     * AJAX handler to delete a settlement by UID.
     *
     * @return void Sends JSON response and exits.
     */
    public static function delete_settlement_ajax()
    {
        ...
        if ($settlement_uid === '') {
            API_Contract::send_error('Falta la liquidación a eliminar.', 'missing_settlement_uid', 422);
        }

        $deleted = class_exists(Data_Layer::class) ? Data_Layer::delete_finance_settlement($settlement_uid) : false;
        if (!$deleted) {
            API_Contract::send_error('No se pudo eliminar la liquidación.', 'finance_settlement_delete_failed', 500);
        }

        API_Contract::send_success(array('message' => 'Liquidación eliminada.'), 'finance_settlement_deleted', 'Liquidación eliminada.', 200);
    }

    /**
     * AJAX handler to delete an expense by UID.
     *
     * @return void Sends JSON response and exits.
     */
    public static function delete_expense_ajax()
    {
        ...
        if ($expense_uid === '') {
            API_Contract::send_error('Falta el gasto a eliminar.', 'missing_expense_uid', 422);
        }

        $deleted = class_exists(Data_Layer::class) ? Data_Layer::delete_finance_expense($expense_uid) : false;
        if (!$deleted) {
            API_Contract::send_error('No se pudo eliminar el gasto.', 'finance_expense_delete_failed', 500);
        }

        API_Contract::send_success(array('message' => 'Gasto eliminado.'), 'finance_expense_deleted', 'Gasto eliminado.', 200);
    }
    ...
    private static function authorize_finance_ajax()
    {
        ...
        if (!current_user_can('manage_options')) {
            API_Contract::send_error('Unauthorized', 'unauthorized', 403);
        }
    }
    ...
    /**
     * Get the default payment channel for a given market.
     *
     * @param string $market Market identifier (mx, us, or manual).
    ...
    /**
     * Get the human-readable label for a market identifier.
     *
    ...
    /**
     * Get the human-readable label for a payment channel.
     *
...
class_alias(Finance::class, 'JPSM_Finance');

...
use JetpackStore\MediaVault\S3_Client;
...
class Config
{
    ...
    public static function test_b2_connection_ajax()
    {
        ...
        if (!class_exists(S3_Client::class)) {
            API_Contract::send_error('MediaVault no disponible.', 'mediavault_unavailable', 500);
        }

        $cfg = self::get_b2_config();
        $valid = self::validate_b2_config($cfg);
        if (is_wp_error($valid)) {
            API_Contract::send_wp_error($valid, $valid->get_error_message(), $valid->get_error_code(), 422);
        }

        $s3 = new S3_Client($cfg['key_id'], $cfg['app_key'], $cfg['region'], $cfg['bucket']);
        $res = $s3->list_objects_page('', null, 1);
        if (is_wp_error($res)) {
            API_Contract::send_error($res->get_error_message(), 'b2_connection_failed', 502);
        }
...
class_alias(Config::class, 'JPSM_Config');

...
class MediaVaultApp {
    constructor() {
        ...
        this.showDemoWelcomeBanner();
    }

    showDemoWelcomeBanner() {
        if ((this.userData.tier ?? -1) !== 0) return;
        const storageKey = 'mv_demo_welcome_dismissed';
        if (localStorage.getItem(storageKey)) return;

        const remaining = this.userData.remainingPlays ?? 15;
        const banner = document.createElement('div');
        banner.id = 'mv-demo-welcome';
        banner.innerHTML = `
            <div class="mv-demo-welcome-inner">
                <div class="mv-demo-welcome-icon">🎧</div>
                <div class="mv-demo-welcome-text">
                    <strong>¡Bienvenido! Tienes ${remaining} reproducciones de cortesía.</strong>
                    <span>Haz clic en ▶ Reproducir en cualquier archivo de audio o video para escuchar una vista previa.</span>
                </div>
                <button class="mv-demo-welcome-close" id="mv-demo-welcome-close" title="Cerrar">&times;</button>
            </div>
        `;

        const mainContent = document.querySelector('.mv-main-content');
        if (!mainContent) return;

        mainContent.insertBefore(banner, mainContent.firstChild);

        const closeBtn = banner.querySelector('#mv-demo-welcome-close');
        if (closeBtn) {
    ...
    fileExtension(value) {
        ...
        // AJAX folder payloads may provide only `f.ext` (for example `mp4`).
        // Treat that bare token as a valid extension so preview buttons stay consistent.
        return /^[a-z0-9]+$/.test(str) ? str : '';
    }
    ...
    bindEvents() {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (btn) {
                ...
                if (card && (card.classList.contains('locked') || card.classList.contains('download-restricted') || card.classList.contains('tier-restricted'))) {
    ...
    applyFolderLocks() {
        ...
        document.querySelectorAll('.jpsm-mv-card').forEach(card => {
            ...
            if (!isAllowed) {
                ...
                if (isDemo) {
                    ...
                    card.classList.add('download-restricted');
                    if (btn) {
                        const label = document.createElement('span');
                        label.className = 'mv-download-restricted-label';
                        label.title = 'Disponible en plan Premium';
                        label.textContent = 'Solo Premium';
                        btn.replaceWith(label);
                    }
                } else {
                    // Paid User without tier access: subtle visual restriction
                    card.classList.add('tier-restricted');
                    if (btn) {
                        btn.innerHTML = type === 'folder' ? 'Mejorar Plan 🚀' : 'Mejorar Plan 🚀';
        ...
        // Show Sticky Bar if Demo — deferred (after 30s of browsing)
        if (isDemo && hasLockedContent) {
            if (!this._stickyBarDeferred) {
                this._stickyBarDeferred = true;
                setTimeout(() => {
    ...
    async getPreviewUrl(path) {
        ...
        if (this.userData.tier === 0 && remaining !== null && remaining >= 0) {
            ...
            const floatingCounter = document.getElementById('mv-demo-plays-count');
            if (floatingCounter) {
                floatingCounter.textContent = remaining;
            }
            // Update sticky bar counter
            const stickyCounter = document.getElementById('mv-sticky-plays-counter');
            if (stickyCounter) {
                stickyCounter.textContent = `${remaining}/15 restantes`;
            }
            // Show sticky bar after first play
            const stickyBar = document.getElementById('mv-sticky-unlock-bar');
            if (stickyBar && !stickyBar.classList.contains('visible')) {
                stickyBar.classList.add('visible');
            }
                ...
                if (grid) {
                    ...
                    if (foldersMap.size > 0) {
                        ...
                        foldersMap.forEach(folder => {
                            ...
                            const html = `
                                <div class="jpsm-mv-card mv-item-folder" data-name="${safeFolderNameAttr}" data-type="folder" data-path="${safeFolderPath}">
                    ...
                    if (filesList.length > 0) {
                        ...
                        filesList.forEach(file => {
                            ...
                            const html = `
                            <div class="jpsm-mv-card" data-name="${safeName}" data-bpm="${Number.isFinite(bpm) ? bpm : 0}" data-type="file" data-path="${safePath}">
                                <div class="jpsm-mv-cover" style="display:none !important;"></div>
                                ...
                                <div style="display:flex; gap:8px; margin-left:auto; flex-shrink: 0;">
                                    ...
                                    <button type="button" class="mv-preview-btn" data-path="${safePath}"
                                        data-name="${safeName}" data-type="${mvEscapeAttr(ext)}">&#9654; Reproducir</button>
                                    ` : ''}
                                    ${(userTier > 0) ? `
    ...
    renderGrid(data) {
        ...
        data.folders.forEach(f => {
            ...
            if (currentDepth >= 1) {
                ...
                if (f && f.can_download) {
                    ...
                    actionBtn = `<span class="mv-download-restricted-label" title="Disponible en plan Premium">Solo Premium</span>`;
                }
            } else {
                actionBtn = `
        ...
        data.files.forEach(f => {
            ...
            if (f && f.can_download) {
                ...
                actionBtns += `<span class="mv-download-restricted-label" title="Disponible en plan Premium">Solo Premium</span>`;
            }

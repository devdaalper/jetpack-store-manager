...
class UI
{
    ...
    private static function handle_pre_output_requests()
    {
        ...
        if (isset($_POST['jdd_login']) && isset($_POST['jdd_email'])) {
            ...
            $ok = Access_Manager::set_access_cookie($email);
            if (!$ok) {
     ...
     * @param S3_Client $s3 The S3 client instance
    ...
    {
        if (class_exists(Index_Manager::class) && method_exists(Index_Manager::class, 'has_index_data') && Index_Manager::has_index_data()) {
    ...
    private static function track_behavior_event_passive($event_name, $fields = array())
    {
        ...
        try {
            ...
            Behavior_Service::track_event_passive($event_name, $payload);
        } catch (\Throwable $t) {
    ...
    public static function handle_ajax()
    {
        ...
        if (($is_mv_ajax_payload || in_array($action, $nonce_required_actions, true)) && class_exists(Auth::class)) {
            $nonce_ok = Auth::validate_request_nonce(array('jpsm_nonce', 'jpsm_mediavault_nonce', 'jpsm_access_nonce'));
            if (!$nonce_ok) {
                API_Contract::send_error('Invalid nonce');
            }
        ...
        if (!self::is_frontend_admin_panel_enabled() && in_array($action, $frontend_admin_actions, true)) {
            API_Contract::send_error('No autorizado', 'forbidden', 403);
        }
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'mv_list_folder') {
            ...
            if (!Access_Manager::check_current_session()) {
                API_Contract::send_error('Sesión no válida');
            }
            ...
            if ($target_folder === '' && isset($_GET['folder']) && (string) $_GET['folder'] !== '') {
                API_Contract::send_error('Carpeta inválida.');
            }
            ...
            $current_email = Access_Manager::get_current_email();
            $user_tier = $current_email ? Access_Manager::get_user_tier($current_email) : 0; // Default to Demo (0)
            ...
            if ($user_tier <= 0) {
                ...
                API_Contract::send_error('🔒 Descarga disponible solo en plan Premium.');
            }

            if (!Access_Manager::user_can_access($target_folder, $user_tier)) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                ...
                API_Contract::send_error('⛔ Acceso Denegado: No tienes permiso para descargar esta carpeta.');
            }
            // ----------------------
            ...
            if (is_wp_error($files_list)) {
                ...
                API_Contract::send_error($files_list->get_error_message());
            }
            ...
            foreach ($files_list as $f) {
                ...
                if ($file_folder !== '' && !Access_Manager::user_can_access($file_folder, $user_tier)) {
                    continue;
                }

                $download_url = $s3->get_presigned_url($f['path']);
                if (class_exists(Config::class)) {
                    $download_url = Config::rewrite_download_url_for_cloudflare((string) $download_url);
                }
            ...
            if (!empty($response) && class_exists(Data_Layer::class)) {
                try {
                    Data_Layer::record_folder_download_event($target_folder);
                } catch (\Throwable $t) {
                    if (defined('WP_DEBUG') && WP_DEBUG) {
            ...
            API_Contract::send_success($response);
            exit;
        }
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'mv_search_global') {
            try {
                ...
                if (empty($query)) {
                    if ($api_version >= 2) {
                        $stats = class_exists(Index_Manager::class) ? Index_Manager::get_stats() : array();
                        $index_state = 'empty';
                ...
                $search_payload = Index_Manager::search_v2($query, $type_filter, $limit, $offset, true, $bpm_min, $bpm_max);
                $results = (array) ($search_payload['items'] ?? array());

                // --- SECURITY & FILTERING ---
                $current_email = Access_Manager::get_current_email();
                $user_tier = $current_email ? Access_Manager::get_user_tier($current_email) : 0;
                ...
                foreach ($results as $r) {
                    ...
                    $can_download = ($user_tier > 0) && Access_Manager::user_can_access($folder_path, $user_tier);
                    $response[] = [
                        'name' => $r['name'],
                        'path' => $r['path'],
                        'size' => (int) $r['size'],
                ...
                API_Contract::send_error('Error interno: ' . $e->getMessage());
            } catch (\Throwable $t) {
                error_log('[MediaVault] Global Search Fatal: ' . $t->getMessage());
                API_Contract::send_error('Error fatal: ' . $t->getMessage());
            }
            exit;
        }
        ...
        if (isset($_REQUEST['action']) && $_REQUEST['action'] === 'mv_track_mobile_notice_event') {
            ...
            if (!Access_Manager::check_current_session()) {
                API_Contract::send_error('Sesión no válida', 'invalid_session', 403);
            }
            ...
            if (is_wp_error($tracked)) {
                API_Contract::send_wp_error($tracked, 'Evento inválido', 'invalid_event', 422);
            }

            API_Contract::send_success($tracked, 'ok', 'Evento registrado');
            exit;
        }

        // Sync Index Endpoint (admin only)
        if (isset($_GET['action']) && $_GET['action'] === 'mv_sync_index') {
            $has_access = class_exists(Auth::class) ? Auth::is_admin_authenticated(true) : false;

            if (!$has_access) {
                API_Contract::send_error('No autorizado');
            }

            $result = Index_Manager::sync_from_s3();
            // Clear folder cache so changes in S3 are reflected immediately
            self::clear_folder_cache();
            if (is_wp_error($result)) {
                API_Contract::send_error($result->get_error_message());
            }
            API_Contract::send_success($result);
            exit;
        }
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'mv_index_stats') {
            ...
            API_Contract::send_success($stats);
            exit;
        }
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'mv_get_presigned_url') {
            ...
            if (class_exists(Config::class) && !Config::is_b2_configured()) {
                API_Contract::send_error('MediaVault no está configurado.', 'missing_config', 503);
            }

            $path_raw = isset($_REQUEST['path']) ? sanitize_text_field(wp_unslash($_REQUEST['path'])) : '';
            $path = self::normalize_object_key($path_raw);
            if ($path === '' && $path_raw !== '') {
                API_Contract::send_error('Path inválido.', 'invalid_path', 400);
            }
            if ($path === '' || substr($path, -1) === '/') {
                API_Contract::send_error('Path requerido.', 'missing_path', 422);
            }
            ...
            if ($user_tier <= 0) {
                ...
                API_Contract::send_error('🔒 Descarga disponible solo en plan Premium.', 'requires_premium', 403);
            }

            $folder_path = self::folder_from_object_key($path);
            if (!Access_Manager::user_can_access($folder_path, $user_tier)) {
                self::track_behavior_event_passive('download_file_denied', array(
                    'object_type' => 'file',
                    'object_path_norm' => $path,
                    'status' => 'denied',
                ...
                API_Contract::send_error('⛔ Acceso Denegado: No tienes permiso para descargar este archivo.', 'access_denied', 403);
            }
            ...
            if (!$presigned_url) {
                ...
                API_Contract::send_error('Error generando URL.', 'signing_failed', 500);
            }
            if (class_exists(Config::class)) {
                $presigned_url = Config::rewrite_download_url_for_cloudflare((string) $presigned_url);
            }
            ...
            API_Contract::send_success(['url' => $presigned_url]);
            exit;
        }
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'mv_get_preview_url') {
            ...
            $remaining_plays = -1;
            if ($user_tier <= 0) {
                if (!Access_Manager::can_play($current_email)) {
                    API_Contract::send_error('Límite de vistas previas alcanzado', 'limit_reached', 403);
                }
                $meta = Access_Manager::log_play($current_email); // increments + returns remaining
                $remaining_plays = isset($meta['remaining']) ? (int) $meta['remaining'] : 0;
            } else {
                $remaining_plays = (int) Access_Manager::get_remaining_plays($current_email);
            }
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'mv_stream_preview') {
            ...
            $current_email = strtolower((string) Access_Manager::get_current_email());
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'mv_get_user_meta') {
            ...
            $has_access = class_exists(Auth::class) ? Auth::is_admin_authenticated(false) : current_user_can('manage_options');
            if (!$has_access) {
                API_Contract::send_error('No autorizado');
            }
            ...
            if (empty($target_email)) {
                API_Contract::send_error('Email requerido');
            }

            $tier = Access_Manager::get_user_tier($target_email);
            $plays = Access_Manager::get_play_count($target_email);
            $is_customer = Access_Manager::is_customer($target_email);

            API_Contract::send_success([
                'email' => $target_email,
                'tier' => $tier,
                'tier_name' => Access_Manager::get_tier_name($tier),
                'plays' => $plays,
                'remaining_plays' => Access_Manager::get_remaining_plays($target_email),
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'mv_update_tier') {
            ...
            Access_Manager::set_user_tier($target_email, $new_tier);

            API_Contract::send_success([
                'email' => $target_email,
                'tier' => $new_tier,
                'tier_name' => Access_Manager::get_tier_name($new_tier)
            ]);
            exit;
        }
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'mv_get_folders') {
            ...
            API_Contract::send_success(Access_Manager::get_all_folder_permissions());
            exit;
        }
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'mv_update_folder') {
            ...
            if (empty($folder_path)) {
                API_Contract::send_error('Carpeta requerida');
            }
            ...
            Access_Manager::set_folder_tier($folder_path, $tier);

            API_Contract::send_success(['folder' => $folder_path, 'tier' => $tier]);
            exit;
        }
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'mv_get_leads') {
            ...
            API_Contract::send_success(Access_Manager::get_leads());
            exit;
        }
    }
    ...
    public static function render()
    {
        ...
        $user_email = Access_Manager::get_current_email();
        $user_tier = Access_Manager::get_user_tier($user_email);
        // Used by both initial render and mv_ajax JSON payload; define early to keep mv_ajax responses clean.
        $remaining_plays = Access_Manager::get_remaining_plays($user_email);
        ...
        if ($is_home_screen && class_exists(Nav_Order::class) && isset($data['folders']) && is_array($data['folders'])) {
            $data['folders'] = Nav_Order::apply_order($data['folders']);
        }
        ...
        if (isset($_GET['mv_ajax']) && $_GET['mv_ajax'] == '1') {
            ...
            $response = [
                ...
                'user_tier_name' => Access_Manager::get_tier_name($user_tier),
            ...
            if ($is_home_screen && class_exists(Nav_Order::class)) {
                $folders_for_response = Nav_Order::apply_order($folders_for_response);
            }

            foreach ($folders_for_response as $folder) {
                $folder_name = trim($folder, '/');
                $display_name = basename($folder_name);
                $can_download_folder = ($user_tier > 0) && Access_Manager::user_can_access($folder, $user_tier);
                // Cover Logic
            ...
            foreach ($data['files'] as $file) {
                ...
                $can_download = ($user_tier > 0) && Access_Manager::user_can_access($file_folder, $user_tier);
        ...
        // 5. Usage stats
        $usage = Traffic_Manager::get_daily_usage($user_email);
        $limit_reached = !Traffic_Manager::can_download($user_email);
        ...
        if (class_exists(Nav_Order::class) && is_array($sidebar_folders)) {
            $sidebar_folders = Nav_Order::apply_order($sidebar_folders);
        }
        $effective_root_path = $junction_folder;
        $force_list_view = wp_is_mobile();
        ...
        <style>
            ...
            /* Demo user: card looks normal, only download is restricted */
            .jpsm-mv-card.download-restricted {
                opacity: 1;
            }
            .jpsm-mv-card.download-restricted .jpsm-mv-cover-icon,
            .jpsm-mv-card.download-restricted .jpsm-mv-cover img {
                filter: none;
                opacity: 1;
            }
            .jpsm-mv-card.download-restricted .locked-icon {
                display: none !important;
            }
            .jpsm-mv-card.download-restricted .mv-preview-btn {
                flex: 2;
                font-size: 14px;
                padding: 10px 16px;
            }

            /* Paid user without tier access: subtle restriction */
            .jpsm-mv-card.tier-restricted {
                opacity: 0.85;
            }
            .jpsm-mv-card.tier-restricted .jpsm-mv-cover-icon,
            .jpsm-mv-card.tier-restricted .jpsm-mv-cover img {
                filter: grayscale(0.3);
                opacity: 0.7;
            }
            .jpsm-mv-card.tier-restricted .locked-icon {
                display: none !important;
            }

            /* Subtle label replacing the locked download button */
            .mv-download-restricted-label {
                font-size: 11px;
                color: #94a3b8;
                background: #f1f5f9;
            ...
            .mv-demo-welcome-icon {
                font-size: 32px;
                flex-shrink: 0;
            }
            .mv-demo-welcome-text {
                flex: 1;
                font-size: 14px;
                color: #1e3a5f;
                line-height: 1.5;
            }
            .mv-demo-welcome-text strong {
                display: block;
                font-size: 15px;
                margin-bottom: 4px;
                color: #1e40af;
            }
            .mv-demo-welcome-text span {
                color: #475569;
            }
            .mv-demo-welcome-close {
                background: none;
                border: none;
                font-size: 20px;
                color: #94a3b8;
            ...
            /* Floating plays counter for demo users */
            .mv-demo-plays-floating {
                display: inline-flex;
                align-items: center;
                gap: 6px;
                background: #eff6ff;
            ...
            /* Sticky bar plays badge */
            .mv-sticky-plays-badge {
                background: #fff7ed;
                color: #c2410c;
                padding: 4px 12px;
                border-radius: 99px;
        ...
        <div class="jpsm-mv-container">
                    ...
                    <div style="font-size: 0.85rem; color: var(--mv-text-muted); margin-bottom: 8px;">
                        ...
                        ?>
	                        <span
	                            ...
	                            <?php echo esc_html(Access_Manager::get_tier_name($user_tier)); ?>
	                        </span>
                    </div>
                    <?php if ($user_tier == 0): ?>
            ...
            <div class="mv-main-content">
                ...
                <?php
                $email_banner = Access_Manager::get_current_email();
                $tier_banner = Access_Manager::get_user_tier($email_banner);
                if ($tier_banner === 0):
                    ?>
                    ...
                    ">
                        <div style="display:flex; align-items:center; gap:12px;">
                            ...
                            <div>
                                <div style="font-weight:700; font-size:15px;">Modo Demo: Explora y Reproduce</div>
                                <div style="font-size:13px; opacity:0.9;">Haz clic en ▶ Reproducir para escuchar. Actualiza tu plan para descargar.</div>
                            </div>
                        </div>
                        <?php
                ...
                <?php if ($user_tier === 0): ?>
                <div id="mv-demo-plays-floating" class="mv-demo-plays-floating">
                    ▶ <span id="mv-demo-plays-count"><?php echo max(0, intval($remaining_plays)); ?></span>/15 reproducciones disponibles
                </div>
                <?php endif; ?>
                    ...
                    <?php foreach ($data['folders'] as $folder): ?>
                        ...
                        <div class="jpsm-mv-card mv-item-folder" data-name="<?php echo esc_attr($sort_name); ?>" data-date=""
                            ...
                            <?php if ($folder_depth >= 2): ?>
                                ...
                                <?php
                                $tier_check = Access_Manager::get_user_tier(Access_Manager::get_current_email());
                                if ($tier_check > 0):
                                    ?>
                    ...
                    <?php foreach ($data['files'] as $file): ?>
                        ...
                        <div class="jpsm-mv-card mv-item-file" data-name="<?php echo esc_attr(strtolower($file['name'])); ?>"
                            ...
                            <div style="display:flex; gap:8px; margin-top:12px; padding: 0 16px;">
                                ...
                                <?php else: ?>
                                    <span class="mv-download-restricted-label" title="Disponible en plan Premium">Solo Premium</span>
                                <?php endif; ?>
                            </div>
        ...
        <div id="mv-sticky-unlock-bar" class="mv-sticky-unlock-bar">
            ...
            <div class="mv-sticky-text">
                <strong>¿Te gusta lo que escuchas?</strong><br>
                <span style="font-size:12px;opacity:0.8;">Obtén descargas ilimitadas y acceso completo.</span>
            </div>
            <span id="mv-sticky-plays-counter" class="mv-sticky-plays-badge"></span>
            <button class="mv-sticky-btn" id="mv-sticky-cta">Obtener acceso completo</button>
        </div>
        ...
        $current_email = Access_Manager::get_current_email();
        $user_tier = $current_email ? Access_Manager::get_user_tier($current_email) : 0;
        $is_admin = class_exists(Auth::class) ? Auth::is_admin_authenticated(false) : false;
        $frontend_admin_panel_enabled = self::is_frontend_admin_panel_enabled();
        $remaining_plays = $current_email ? Access_Manager::get_remaining_plays($current_email) : 0;
        ...
        ?>
	        ...
	        $mv_user_data = [
	            ...
	            'tierName' => Access_Manager::get_tier_name($user_tier),
	            ...
	            'folderPerms' => Access_Manager::get_all_folder_permissions(),
	            ...
	            'mediaExtensions' => class_exists(Index_Manager::class)
	                ? Index_Manager::get_media_extension_map()
	                : array(
...
class_alias(UI::class, 'JPSM_MediaVault_UI');

...
class Index_Manager
{
    ...
    /**
     * Sanitize a table name for safe interpolation in SQL.
     *
     * All table names in this class come from internal constants combined with
     * $wpdb->prefix, so they are safe by construction. This wrapper adds
     * defense-in-depth via esc_sql() and backtick-quoting so that static
     * analysis and code review can verify safety at the call site.
     *
     * @param string $table Raw table name from get_table_name() or similar.
     * @return string Escaped and backtick-quoted identifier, e.g. `wp_jpsm_mediavault_index`.
     */
    private static function safe_table($table)
    {
    ...
    private static function drop_legacy_unique_path_index($table_name)
    {
        ...
        $safe = self::safe_table($table_name);

        if (!self::is_sqlite_db()) {
            $mysql_idx = $wpdb->get_row(
                $wpdb->prepare(
                    "SHOW INDEX FROM $safe WHERE Key_name = %s",
                    'path'
                ),
                ARRAY_A
            );
            if (is_array($mysql_idx) && isset($mysql_idx['Non_unique']) && intval($mysql_idx['Non_unique']) === 0) {
                $wpdb->query("ALTER TABLE $safe DROP INDEX path");
            }
            return;
        }

        // SQLite path (wp-now integration).
        $esc_name = esc_sql($table_name);
        $idx_rows = $wpdb->get_results("PRAGMA index_list('$esc_name')", ARRAY_A);
        ...
        foreach ($idx_rows as $idx) {
            ...
            $wpdb->query("DROP INDEX IF EXISTS `" . esc_sql($idx_name) . "`");
            break;
        }
    }
    ...
    public static function search_v2($query, $type_filter = null, $limit = 100, $offset = 0, $with_fuzzy = true, $bpm_min = null, $bpm_max = null)
    {
        ...
        $table_name = self::safe_table(self::get_table_name('active'));
    ...
    public static function has_index_data($role = 'active')
    {
        ...
        $safe = self::safe_table($table_name);
        $has = (bool) $wpdb->get_var("SELECT 1 FROM $safe LIMIT 1");
    ...
    public static function list_folder_structure($folder)
    {
        ...
        $safe_table = self::safe_table(self::get_table_name('active'));
        ...
        if ($folder_key === '') {
            $folders_rows = $wpdb->get_col("SELECT DISTINCT folder FROM $safe_table WHERE folder NOT IN ('.','') ORDER BY folder ASC");
        } else {
            $like_folder = $wpdb->esc_like($folder_key) . '/%';
            $folders_rows = $wpdb->get_col(
                $wpdb->prepare(
                    "SELECT DISTINCT folder FROM $safe_table WHERE folder LIKE %s ORDER BY folder ASC",
                    $like_folder
                )
            );
        }
    ...
    public static function get_stats()
    {
        ...
        $active_table = self::safe_table(self::get_table_name('active'));
    ...
    public static function get_stats_ajax()
    {
        ...
        API_Contract::send_success($stats, 'index_stats_fetched', 'Estadísticas del índice cargadas.', 200);
    }
    ...
    public static function sync_mediavault_index_ajax()
    {
        ...
        try {
            ...
            if (is_wp_error($result)) {
                API_Contract::send_wp_error($result, 'Error de sincronización', 'index_sync_failed', 502);
                return;
            }
            if (isset($result['success']) && !$result['success']) {
                API_Contract::send_error($result['error'] ?? 'Error de sincronización', 'index_sync_failed', 500, $result);
                return;
            }

            API_Contract::send_success($result, 'index_sync_batch_completed', 'Lote de sincronización procesado.', 200);
        } catch (Exception $e) {
            error_log('MediaVault Sync Exception: ' . $e->getMessage());
            API_Contract::send_error('Server Error: ' . $e->getMessage(), 'server_error', 500);
        } catch (Error $e) {
            error_log('MediaVault Sync Fatal Error: ' . $e->getMessage());
            API_Contract::send_error('Fatal Error: ' . $e->getMessage(), 'fatal_error', 500);
        }
    }
    ...
    public static function auto_detect_bpm_batch_ajax()
    {
        ...
        if (is_wp_error($result)) {
            API_Contract::send_wp_error($result, 'No se pudo procesar la extracción BPM automática.', 'auto_bpm_failed', 502);
            return;
        }

        API_Contract::send_success($result, 'auto_bpm_batch_completed', 'Lote BPM automático procesado.', 200);
    }
    ...
    public static function desktop_issue_token_ajax()
    {
        ...
        try {
            ...
            API_Contract::send_error('No se pudo generar token desktop.', 'token_issue_failed', 500);
            return;
        }
    ...
    public static function desktop_api_health_ajax()
    {
        ...
        if (!self::verify_desktop_token($token)) {
            API_Contract::send_error('Unauthorized', 'unauthorized', 401);
            return;
        }
    ...
    public static function import_bpm_batch_api_ajax()
    {
        ...
        if (is_wp_error($payload)) {
            API_Contract::send_wp_error($payload, 'Body JSON inválido.', 'invalid_json', 422);
            return;
        }

        $batch_id = sanitize_text_field((string) ($payload['batch_id'] ?? ''));
        if ($batch_id === '' || strlen($batch_id) > 64) {
            API_Contract::send_error('batch_id inválido.', 'invalid_batch_id', 422);
            return;
        }
        ...
        if (empty($rows)) {
            API_Contract::send_error('rows requerido.', 'missing_rows', 422);
            return;
        }
        if (count($rows) > 5000) {
            API_Contract::send_error('rows excede máximo por lote (5000).', 'rows_limit_exceeded', 422);
            return;
        }
        ...
        if (is_array($existing_batch)) {
            ...
            API_Contract::send_error('batch_id ya existe con payload distinto.', 'batch_conflict', 409);
            return;
        }
    ...
    public static function rollback_bpm_batch_api_ajax()
    {
        ...
        $batch_id = sanitize_text_field((string) ($payload['batch_id'] ?? ''));
        if ($batch_id === '') {
            API_Contract::send_error('batch_id requerido.', 'missing_batch_id', 422);
            return;
        }

        $batch = self::get_bpm_batch($batch_id);
        if (!is_array($batch)) {
            API_Contract::send_error('batch_id no encontrado.', 'batch_not_found', 404);
            return;
        }

        $rows = self::get_bpm_batch_rows($batch_id);
        if (empty($rows)) {
            API_Contract::send_error('No hay filas para revertir en ese batch.', 'empty_batch_rows', 409);
            return;
        }
    ...
    public static function import_bpm_csv_ajax()
    {
        ...
        if (empty($_FILES['bpm_csv']) || !is_array($_FILES['bpm_csv'])) {
            API_Contract::send_error('Archivo CSV requerido.', 'missing_file', 422);
            return;
        }
        ...
        if ($error_code !== UPLOAD_ERR_OK || $tmp_name === '' || !file_exists($tmp_name)) {
            API_Contract::send_error('No se pudo leer el archivo CSV.', 'invalid_upload', 422);
            return;
        }
        ...
        $fh = fopen($tmp_name, 'r');
        if ($fh === false) {
            API_Contract::send_error('No se pudo abrir el archivo CSV.', 'open_failed', 422);
            return;
        }
        ...
        if (!is_array($first)) {
            fclose($fh);
            API_Contract::send_error('CSV vacío.', 'empty_csv', 422);
            return;
        }
        $row_num++;
        ...
        if ($has_header) {
            ...
            if (isset($header_map['bpm'])) {
                ...
                fclose($fh);
                API_Contract::send_error('CSV inválido: falta columna bpm/tempo.', 'invalid_csv_header', 422);
                return;
            }
        } else {
    ...
    private static function fetch_auto_bpm_candidates($limit)
    {
        ...
        $table_raw = self::get_table_name('active');
        if (!self::table_exists($table_raw)) {
            return new WP_Error('index_missing', 'MediaVault index table not found');
        }

        $table = self::safe_table($table_raw);
        $audio_exts = "'" . implode("','", self::$media_extensions['audio']) . "'";
    ...
    private static function reset_auto_scan_marks()
    {
        ...
        foreach (array('primary', 'shadow') as $alias) {
            $table_raw = self::get_table_name($alias);
            if (!self::table_exists($table_raw)) {
                continue;
            }
    ...
    private static function get_bpm_override_by_hash($path_hash)
    {
        ...
        $table_raw = self::get_bpm_overrides_table_name();
        if (!self::table_exists($table_raw)) {
            return null;
        }
    ...
    private static function apply_bpm_overrides_to_indexes()
    {
        ...
        $overrides_raw = self::get_bpm_overrides_table_name();
        if (!self::table_exists($overrides_raw)) {
            return array('primary' => 0, 'shadow' => 0);
        }

        $overrides = self::safe_table($overrides_raw);
        $counts = array('primary' => 0, 'shadow' => 0);
        foreach (array('primary', 'shadow') as $alias) {
            $index_table_raw = self::get_table_name($alias);
            if (!self::table_exists($index_table_raw)) {
                continue;
            }

            $index_table = self::safe_table($index_table_raw);
            if (!self::is_sqlite_db()) {
                ...
                $safe_raw = esc_sql($index_table_raw);
                $sql = "UPDATE $index_table AS idx
                        SET bpm = COALESCE((SELECT ovr.bpm FROM $overrides ovr WHERE ovr.path_hash = idx.path_hash), bpm),
                            bpm_source = COALESCE((SELECT ovr.source FROM $overrides ovr WHERE ovr.path_hash = idx.path_hash), bpm_source)
                        WHERE EXISTS (SELECT 1 FROM $overrides ovr WHERE ovr.path_hash = idx.path_hash)";
                $affected = $wpdb->query($sql);
            }
    ...
    private static function get_bpm_batch($batch_id)
    {
        ...
        $table_raw = self::get_bpm_batches_table_name();
        if (!self::table_exists($table_raw)) {
            return null;
        }
    ...
    private static function get_bpm_batch_rows($batch_id)
    {
        ...
        $table_raw = self::get_bpm_batch_rows_table_name();
        if (!self::table_exists($table_raw)) {
            return array();
        }
    ...
    private static function truncate_table($table_name)
    {
        ...
        $safe = self::safe_table($table_name);
        $result = $wpdb->query("TRUNCATE TABLE $safe");
        if ($result === false) {
            $result = $wpdb->query("DELETE FROM $safe");
        }
        return $result;
    }
...
class_alias(Index_Manager::class, 'JPSM_Index_Manager');
